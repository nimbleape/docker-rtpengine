...
if [ "$1" = 'rtpengine' ]; then
  shift
  echo "rtpengine --interface=$MY_IP --foreground --log-stderr --port-min=$MIN_PORT --port-max=$MAX_PORT --recording-dir=$RECORDING_DIR --recording-method=$RECORDING_METHOD --recording-format=$RECORDING_FORMAT --log-level=$LOG_LEVEL --delete-delay=$DELETE_DELAY --listen-http=$HTTP_PORT $@"
